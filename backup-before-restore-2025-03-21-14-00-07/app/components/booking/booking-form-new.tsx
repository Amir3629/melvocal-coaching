...
export default function BookingForm() {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      // Handle closing with animation before redirecting
      handleSmoothClose(() => {
  ...
  // Handle smooth closing animation with callback
  const handleSmoothClose = (onAnimationComplete?: () => void) => {
  ...
  // Custom onClose function for the confirmation step
  const handleConfirmationClose = () => {
    // Use same animation timing as in the confirmation step
    setIsVisible(false)
    
    // After booking form closes, scroll to top
    setTimeout(() => {
      window.scrollTo({ top: 0, behavior: 'smooth' })
    }, 800)
  }
    ...
    <AnimatePresence>
      {isVisible && (
        ...
        >
          ...
          <motion.div 
            className="bg-[#121212] border border-gray-800 rounded-xl p-6 mb-6"
