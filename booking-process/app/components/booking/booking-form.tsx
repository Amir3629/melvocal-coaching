...
export default function BookingForm() {
  ...
  const handleNextStep = () => {
    if (currentStep < 3) {
      ...
      // Show calendar on step 2 for specific services
      if (currentStep === 1 && (selectedService === "vocal-coaching" || selectedService === "gesangsunterricht")) {
  ...
  const handlePrevStep = () => {
    if (currentStep > 0) {
      setCurrentStep(currentStep - 1)
      // Show calendar on step 2 for specific services
      if (currentStep === 3 && (selectedService === "vocal-coaching" || selectedService === "gesangsunterricht")) {
        setShowCalendar(true)
      } else {
        setShowCalendar(false)
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert(t("booking.errorSubmitting", "Fehler beim Absenden der Buchung. Bitte versuchen Sie es später erneut."))
    } finally {
      ...
      case 2:
        ...
        switch (selectedService) {
          case "professioneller-gesang":
            return <LiveSingingForm formData={formData} onChange={handleFormChange} />
          case "vocal-coaching":
            return <VocalCoachingForm formData={formData} onChange={handleFormChange} />
          case "gesangsunterricht":
            return <WorkshopForm formData={formData} onChange={handleFormChange} />
          default:
            return null
        }
      case 3:
        return <ConfirmationStep formData={formData} serviceType={selectedService} onChange={handleFormChange} />
      default:
        return null
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6">
      ...
      <div className="mb-6">
        <h2 className="text-xl sm:text-2xl font-bold text-white">{getStepTitle()}</h2>
        <p className="text-gray-400 mt-2 text-sm sm:text-base">
          ...
          {currentStep === 3 &&
            t("booking.confirmationDesc", "Überprüfen Sie Ihre Angaben und senden Sie die Anfrage ab.")}
        </p>
      </div>
      ...
      <div className="mt-6 mb-8 flex justify-between items-center">
        ...
        <div className={currentStep === 0 ? "w-full sm:w-auto" : "w-auto"}>
          {currentStep < 3 ? (
            <button
              type="button"
              ...
              className={`px-4 sm:px-6 py-2 rounded-lg transition-colors ${
                currentStep === 0 ? "w-full sm:w-auto" : "w-auto"
              } ${
                isStepValid()
            ...
            <SubmitButton onClick={handleSubmit} disabled={!isStepValid() || isSubmitting} isLoading={isSubmitting} />
          )}
        </div>
      </div>
    </div>
